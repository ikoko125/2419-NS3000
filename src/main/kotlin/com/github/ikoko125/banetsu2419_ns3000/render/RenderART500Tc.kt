...
import com.github.ikoko125.banetsu2419_ns3000.model.ModelART500Tc
...
object RenderART500Tc {
    ...
    private fun renderRandomCurtain(renderer: VehiclePartsRenderer) {
        ...
        ModelART500Tc.getModel(renderer.modelName)!!.nightWindow.render(renderer)
    }

    private fun renderLights(renderer: VehiclePartsRenderer, entityTrainBase: EntityTrainBase) {
        val model = ModelART500Tc.getModel(renderer.modelName)!!

        when (entityTrainBase.getVehicleState(TrainState.TrainStateType.Role).toInt()) {
            0 -> model.aOff.render(renderer)
            2 -> model.aOffB.render(renderer)
        }
    }
}
